  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
      ...
      <header className="mb-16">
        <h1 className="text-5xl md:text-7xl font-light serif mb-4">{project.title}</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-16 mb-20">
        <div className="md:col-span-2">
          <h2 className="text-sm tracking-[0.3em] uppercase mb-8 border-b border-neutral-100 pb-2">設計概念 Concept</h2>
          ...
          <div className="mt-12 p-8 bg-neutral-50 border border-neutral-100 italic">
            ...
            {loadingInsight ? (
              ...
              <p className="text-neutral-600 text-sm leading-relaxed">{insight}</p>
            )}
          </div>
        </div>
        ...
        <div className="space-y-12">
          <section>
            <h2 className="text-sm tracking-[0.3em] uppercase mb-8 border-b border-neutral-100 pb-2">專案細節 Data</h2>
            <dl className="space-y-4">
              {project.stats.map((stat, idx) => (
                <div key={idx} className="flex justify-between border-b border-neutral-50 pb-2">
                  <dt className="text-xs text-neutral-400 uppercase tracking-widest">{stat.label}</dt>
      ...
      {project.gallery.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {project.gallery.map((img, idx) => (
            <img key={idx} src={img} className="w-full h-auto grayscale hover:grayscale-0 transition-all duration-700" alt={`Gallery ${idx}`} />
          ))}
        </div>
      )}
    </div>
